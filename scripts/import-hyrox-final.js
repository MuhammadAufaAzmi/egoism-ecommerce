...
const rootHyroxDir = "C:\\Users\\Budiman\\Downloads\\HYROX-20260610T100215Z-3-001\\HYROX";
...
const keywordToFitType = [
  ...
  { key: "specialist", val: "Regular Tshirt" },
  { key: "tshirt", val: "Regular Tshirt" }
];
...
async function main() {
  console.log("🚀 Starting FINAL HYROX Import...");
  let processedCount = 0;

  const mainFolders = fs.readdirSync(rootHyroxDir).filter(f => fs.statSync(path.join(rootHyroxDir, f)).isDirectory());

  for (const mainFolder of mainFolders) {
    const mainFolderPath = path.join(rootHyroxDir, mainFolder);
    const subFolders = fs.readdirSync(mainFolderPath).filter(f => fs.statSync(path.join(mainFolderPath, f)).isDirectory());
    ...
    for (const subFolder of subFolders) {
      ...
      const baseName = mainFolder.toUpperCase().trim();
  ...
  // Restore the 8 base blank products just in case they were lost
  console.log("\n🌱 Seeding base blank products just in case...");
  try {
    const seedScript = path.join(__dirname, '..', 'prisma', 'seed-products.js');
    if (fs.existsSync(seedScript)) {
       require('child_process').execSync(`node ${seedScript}`, { stdio: 'inherit' });
    }
  } catch (e) {
    console.log("Seed blank products skipped.");
  }

  console.log(`\n🎉 SUCCESSFULLY CREATED ${processedCount} HYROX PRODUCTS!`);
}
