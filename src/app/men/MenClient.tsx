...
export default function MenClient({ initialProducts }: MenClientProps) {
  ...
  const processedProducts = useMemo(() => {
    ...
    return result;
  }, [initialProducts, sortBy, filterSize, filterFitType, filterActivity]);

  // Label aktif untuk filter
  const activeFilterLabel = () => {
    const parts: string[] = [];
    if (filterFitType !== "ALL") parts.push(FIT_TYPES.find(f => f.key === filterFitType)?.label || filterFitType);
        ...
        <div className="flex gap-6 relative items-center">
          ...
          <div className="relative">
            <button
              ...
              className={`text-[12px] leading-[16px] tracking-[0.1em] font-semibold transition-colors uppercase ${isFilterOpen || filterSize !== "ALL" || filterFitType !== "ALL" ? "text-primary" : "text-secondary hover:text-primary"}`}
            >
            ...
            {isFilterOpen && (
              <div className="absolute top-full right-0 mt-4 bg-surface-container-lowest border border-outline-variant p-5 z-50 min-w-[180px] shadow-2xl">
                ...
                {/* Reset Filter */}
                {(filterSize !== "ALL" || filterFitType !== "ALL") && (
                  <button
                    onClick={() => {
                      setFilterSize("ALL");
                      setFilterFitType("ALL");
